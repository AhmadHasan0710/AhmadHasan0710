<h1>
  Hello there, I'm Ahmad Hasan!<br>
  $${\color{red}Future \space Network \space and \space Cybersecurity \space Enngineer}$$
</h1>

<h2>
  👨🏽‍💻 Network Developer Projects:
</h3><br>

<h3>
$${\color{red}Network \space Infrastructure \space Homelab}$$
</h3>

•Implemented inter-VLAN routing for segmented network communication<br>
•Designed and secured multiple network segments for traffic isolation and access control<br>
•Utilized pfSense to manage routing, firewall policies, and network stability<br>
<br>

[![Network Infrastructure Homelab](https://img.shields.io/badge/Network%20Infrastructure%20Homelab-red)](https://github.com/AhmadHasan0710/Network-Infrastrucutre-Homelab)

<h3>
$${\color{red}Credential \space Harvesting \space Lab}$$
</h3>

• Social engineering attack simulation (SET framework)<br> 
• Phishing payload behavior analysis<br>
• Network traffic inspection and documentation (packet-level analysis)<br>
<br>

[![Credential Harvesting Lab](https://img.shields.io/badge/Credential%20Harvesting%20Lab-red)](https://github.com/AhmadHasan0710/Simulated-Credential-Harvesting)

<h3>
$${\color{red}All \space Other \space Projects}$$
</h3>

[![AWS VPC](https://img.shields.io/badge/AWS%20TVPC%20Architecture-red)](https://github.com/AhmadHasan0710/Secure-AWS-VPC-Network-Architecture)<br>
&nbsp;&nbsp;&nbsp;- Built isolated cloud subnets with controlled secure traffic flow<br>

[![Zero Trust](https://img.shields.io/badge/Security-Zero%20Trust%20Network-red)](https://github.com/AhmadHasan0710/Zero-Trust-Enterprise-Network)<br>
&nbsp;&nbsp;&nbsp;- Enforced strict access controls across segmented VLAN networks<br>

[![SOC Lab](https://img.shields.io/badge/SOC-Network%20Threat%20Detection-red)](https://github.com/AhmadHasan0710/Soc-Siem-Integration)<br>
&nbsp;&nbsp;&nbsp;- Monitored network activity and detected simulated security threats<br>

<h2>
  📖 Network Education & Certification(s):
</h3><br>

<h3>
$${\color{red}Associates \space Cybersecurity \space Degree}$$
</h3>

•Completed: August 2023 – May 2025  <br>
•Graduated from Hillsborough Community College  <br>
•Currently enrolled in a Bachelor’s in Cybersecurity program at St. Petersburg College  <br>

<h3>
$${\color{red}Current \space Certifications}$$
</h3>

•Cisco Certified Network Associate (CCNA) - Completed August 2026 <br>
• Strong foundation in routing, switching, subnetting, VLANs, network security, & more.  <br>

<h3>
$${\color{red}Future \space Planned \space Certificatoins}$$
</h3>

• CompTIA Security+ — Foundational cybersecurity, risk management, and threat mitigation   <br>
• AWS Certified Cloud Practitioner — Cloud fundamentals and AWS services   <br>
• CompTIA Linux+ — Linux administration and system security   <br>
• Cisco Certified Network Professional (CCNP) — Advanced networking, routing, and enterprise infrastructure   <br>
• Certified Information Systems Security Professional (CISSP) — Advanced security architecture and enterprise-level security management   <br>

